...
`}function qe(e){return T(e.fontUrl)}function Je(e){const t=(e.buttons||[]).map(B).join(""),r=(e.leftButtons||[]).map(B).join("");return`<header class="puck-site-header ${e.headerPosition==="full"?"is-full-width":""}" style="${g({background:e.backgroundColor||"rgba(0,0,0,.72)",borderBottom:e.lineColor?`1px solid ${e.lineColor}`:"none",boxShadow:e.lineShadow||"none",width:e.width||"100%",maxWidth:e.maxWidth||"none",margin:e.margin||"0",padding:e.padding||"22px 40px"})}">
  ...
  </header>`}function Ke(e){const t=e.items||[],r=[...t,...t].map(i=>`<span class="song-name" style="${g({color:e.textColor||"var(--cream)",textShadow:e.textShadow||"none",borderColor:e.buttonBorderColor||"rgba(212, 162, 76, 0.25)"})}">${b(i.text)}</span>`).join("");return`<section class="songs-section puck-song-scroll ${e.stretchMode==="full"?"is-full-width":""}" aria-label="Songs We Play" style="${g({background:e.backgroundColor||"linear-gradient(90deg, var(--blue-deep) 0%, var(--charcoal-light) 50%, var(--blue-deep) 100%)",borderBottom:e.lineColor?`1px solid ${e.lineColor}`:"none",width:e.width||"100%",maxWidth:e.maxWidth||"none",margin:e.margin||"0"})}"><div class="songs-scroll-container"><div class="songs-scroll puck-song-track">${r}</div></div></section>`}function Qe(e){const t=e.theme,r=Array.from({length:8}).map(()=>"<span></span>").join(""),i=(e.buttons||[]).map(B).join(""),a=m(e.noThanksText)?`<button class="puck-btn gr-exit-popup-no-thanks" type="button" style="${g({background:e.noThanksBackgroundColor||"transparent",color:e.noThanksTextColor||"#ffffff",border:`1px solid ${e.noThanksBorderColor||"rgba(255,255,255,.45)"}`,borderRadius:"999px",padding:"16px 34px",fontFamily:"Oswald, sans-serif",fontSize:"16px",fontWeight:"700",textTransform:"uppercase",cursor:"pointer"})}">${b(e.noThanksText)}</button>`:"",l=`gr-exit-popup-${Math.random().toString(36).slice(2)}`;return`${T(e.customFontUrl)}
  <style>
